...
1. 加载 .env 文件 (如果存在)
2. 检查 Python 版本和关键依赖
3. 校验 API Key 配置 (未配置时打印友好提示, 不阻断启动)
4. 启动 uvicorn 服务

这个文件是容器内唯一保留 .py 源码的文件 (其余编译为 .pyc),
因此它必须保持自包含, 不依赖 app 包内的其他模块.
"""
...
def _check_python():
    ...
    v = sys.version_info
    if v < (3, 8):
        print(f"[boot] 错误: Python {v.major}.{v.minor} 不支持, 最低需要 3.8")
        sys.exit(1)
    if v < (3, 10):
        print(f"[boot] 提示: Python {v.major}.{v.minor} 可用, 推荐 3.10+ 以获得更好性能")
...
def _check_deps():
    ...
    for pkg in critical:
        try:
            ...
            missing.append(pkg)
    if missing:
        print(f"[boot] 错误: 缺少关键依赖: {', '.join(missing)}")
        print("[boot] 请运行: pip install -r requirements.txt")
        sys.exit(1)
    # litellm 是核心 LLM 接入层
    try:
        import litellm  # noqa: F401
    except ImportError:
        print("[boot] 警告: litellm 未安装, LLM 调用将不可用")
        print("[boot] 请运行: pip install litellm")
...
def main():
    ...
    print(f"[boot] 天衍启动中... http://{host}:{port}/")
    uvicorn.run(
        "app.server:app",
        host=host,
        port=port,
